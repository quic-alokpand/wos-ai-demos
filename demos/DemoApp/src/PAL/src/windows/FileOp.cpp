...
bool pal::FileOp::move(const std::string &currentName,
  ...
  if (stat(newName.c_str(), &st) == 0) {
    if ((st.st_mode & S_IFDIR) != 0) {
      // if newName is directory and overwrite = false, cannot move, return
      // false if newName is directory and overwrite = true, delete it and
      // rename
      if (overwrite == false) {
        return false;
      }
...
bool pal::FileOp::getFileInfo(const std::string &filename,
  ...
  std::string name;
  int32_t lastPathSeparator =
      std::max(static_cast<int32_t>(filename.find_last_of('\\')),
...
static bool getFileInfoListRecursiveImpl(
  ...
  while (entryCount > 0) { // Ensure entry count is positive
    entryCount--;
    ...
    if (pal::FileOp::checkIsDir(curPath)) {
      ...
      if (stat(curPath.c_str(), &st) == 0 && ((st.st_mode & S_IFDIR) != 0) &&
          (!getFileInfoListRecursiveImpl(curPath, filenamePartsList, ignoreDirs,
                                         maxDepth - 1))) {
        return false;
      }
...
std::string pal::FileOp::getCurrentWorkingDirectory() {
  ...
  if (0 == GetCurrentDirectoryA(MAX_PATH, buffer)) {
    DEBUG_MSG("Get current working directory fail! Error code : %d",
              GetLastError());
    return std::string();
  }
