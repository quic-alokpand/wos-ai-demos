...
#define QNN_OP_CFG_VALID(opConfig)                                             \
...
inline const char *
getQnnOpConfigPackageName(const Qnn_OpConfig_t *const opConfig) {
...
inline const char *
getQnnOpConfigTypeName(const Qnn_OpConfig_t *const opConfig) {
...
inline const Qnn_Param_t *
getQnnOpConfigParams(const Qnn_OpConfig_t *const opConfig) {
...
inline const Qnn_Tensor_t *
getQnnOpConfigInputs(const Qnn_OpConfig_t *const opConfig) {
...
inline const Qnn_Tensor_t *
getQnnOpConfigOutputs(const Qnn_OpConfig_t *const opConfig) {
  return getQnnOpConfigOutputs(*opConfig);
}

inline void setQnnOpConfigName(Qnn_OpConfig_t &opConfig,
...
inline void setQnnOpConfigName(Qnn_OpConfig_t *const opConfig,
                               const char *const name) {
...
inline void setQnnOpConfigTypeName(Qnn_OpConfig_t *const opConfig,
                                   const char *const typeName) {
...
inline Qnn_TensorDataFormat_t
getQnnTensorDataFormat(const Qnn_Tensor_t *const tensor) {
...
inline Qnn_QuantizeParams_t
getQnnTensorQuantParams(const Qnn_Tensor_t *const tensor) {
  if (tensor != nullptr) {
...
inline Qnn_TensorMemType_t
getQnnTensorMemType(const Qnn_Tensor_t *const tensor) {
...
inline Qnn_ClientBuffer_t
getQnnTensorClientBuf(const Qnn_Tensor_t *const tensor) {
...
inline void setQnnTensorName(Qnn_Tensor_t *const tensor,
                             const char *const name) {
  setQnnTensorName(*tensor, name);
}

inline void setQnnTensorType(Qnn_Tensor_t &tensor,
                             const Qnn_TensorType_t type) {
  // TensorCompatTest justifies no need to check version
  tensor.v1.type = type;
}

inline void setQnnTensorType(Qnn_Tensor_t *const tensor,
                             const Qnn_TensorType_t type) {
  setQnnTensorType(*tensor, type);
}

inline void setQnnTensorDataFormat(Qnn_Tensor_t &tensor,
                                   const Qnn_TensorDataFormat_t dataFormat) {
  // TensorCompatTest justifies no need to check version
...
inline void setQnnTensorDataType(Qnn_Tensor_t *const tensor,
                                 const Qnn_DataType_t dataType) {
...
inline void setQnnTensorQuantParams(Qnn_Tensor_t *const tensor,
...
inline void setQnnTensorDimensions(Qnn_Tensor_t *const tensor,
                                   uint32_t *const dimensions) {
  setQnnTensorDimensions(*tensor, dimensions);
}

inline void
setQnnTensorIsDynamicDimensions(Qnn_Tensor_t &tensor,
...
inline void setQnnTensorMemType(Qnn_Tensor_t &tensor,
                                const Qnn_TensorMemType_t memType) {
  // TensorCompatTest justifies no need to check version
  tensor.v1.memType = memType;
}

inline void setQnnTensorMemType(Qnn_Tensor_t *const tensor,
                                const Qnn_TensorMemType_t memType) {
...
inline void setQnnTensorClientBuf(Qnn_Tensor_t *const tensor,
                                  const Qnn_ClientBuffer_t clientBuf) {
...
inline void setQnnTensorMemHandle(Qnn_Tensor_t *const tensor,
                                  const Qnn_MemHandle_t handle) {
  setQnnTensorMemHandle(*tensor, handle);
}

inline Qnn_TensorSet_t
createQnnTensorSet(const Qnn_TensorSetVersion_t version) {
...
inline Qnn_Tensor_t *
getQnnTensorSetInputTensors(const Qnn_TensorSet_t &tensorSet) {
  if (tensorSet.version == QNN_TENSOR_SET_VERSION_1) {
...
inline Qnn_Tensor_t *
getQnnTensorSetOutputTensors(const Qnn_TensorSet_t &tensorSet) {
  if (tensorSet.version == QNN_TENSOR_SET_VERSION_1) {
...
#define QNN_TENSOR_GET_IS_DYNAMIC_DIMENSIONS(tensor)                           \
  getQnnTensorIsDynamicDimensions(tensor)
...
#define QNN_TENSOR_SET_SPARSE_PARAMS(tensor, value)                            \
  setQnnTensorSparseParams(tensor, value)
...
#define QNN_TENSORSET_GET_NUM_INPUTS(tensorSet)                                \
  getQnnTensorSetNumInputs(tensorSet)
#define QNN_TENSORSET_GET_INPUT_TENSORS(tensorSet)                             \
  getQnnTensorSetInputTensors(tensorSet)
#define QNN_TENSORSET_GET_NUM_OUTPUTS(tensorSet)                               \
  getQnnTensorSetNumOutputs(tensorSet)
#define QNN_TENSORSET_GET_OUTPUT_TENSORS(tensorSet)                            \
  getQnnTensorSetOutputTensors(tensorSet)
...
inline bool isQnnTensorV1Compatible(const Qnn_OpConfig_t &opConfig) {
  if ((QNN_OP_CFG_GET_INPUTS(opConfig) != NULL) &&
      (QNN_OP_CFG_GET_NUM_INPUTS(opConfig) > 0u)) {
    for (uint32_t tensorIdx = 0u;
         tensorIdx < QNN_OP_CFG_GET_NUM_INPUTS(opConfig); tensorIdx++) {
      if (!isQnnTensorV1Compatible(
              QNN_OP_CFG_GET_INPUTS(opConfig)[tensorIdx])) {
        return false;
      }
    }
  }
  if ((QNN_OP_CFG_GET_OUTPUTS(opConfig) != NULL) &&
      (QNN_OP_CFG_GET_NUM_OUTPUTS(opConfig) > 0u)) {
    for (uint32_t tensorIdx = 0u;
         tensorIdx < QNN_OP_CFG_GET_NUM_OUTPUTS(opConfig); tensorIdx++) {
      if (!isQnnTensorV1Compatible(
              QNN_OP_CFG_GET_OUTPUTS(opConfig)[tensorIdx])) {
        return false;
      }
    }
  }
  if ((QNN_OP_CFG_GET_PARAMS(opConfig) != NULL) &&
      (QNN_OP_CFG_GET_NUM_PARAMS(opConfig) > 0)) {
    for (uint32_t paramIdx = 0u; paramIdx < QNN_OP_CFG_GET_NUM_PARAMS(opConfig);
         paramIdx++) {
