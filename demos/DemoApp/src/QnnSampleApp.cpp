...
sample_app::QnnSampleApp::QnnSampleApp(
    ...
    void *backendLibraryHandle, std::string outputPath, bool debug,
    ...
    std::string cachedBinaryPath, std::string saveBinaryName, bool needKpi)
    : m_model(model), m_qnnFunctionPointers(qnnFunctionPointers),
      m_inputArgs(inputArgs), m_outputPath(outputPath),
      m_saveBinaryName(saveBinaryName), m_cachedBinaryPath(cachedBinaryPath),
      m_debug(debug), m_outputDataType(outputDataType),
      m_inputDataType(inputDataType), m_profilingLevel(profilingLevel),
      m_dumpOutputs(dumpOutputs), m_graphsInfo(nullptr), m_graphsCount(0),
      m_backendLibraryHandle(backendLibraryHandle), m_ioTensor(),
      m_isBackendInitialized(false), m_isContextCreated(false),
      m_profileBackendHandle(nullptr), m_graphConfigsInfo(nullptr),
      m_graphConfigsInfoCount(0), m_logHandle(nullptr),
      m_backendHandle(nullptr), m_deviceHandle(nullptr), m_needKpi(needKpi),
      powerConfigId(0), deviceId(0), coreId(0) { // Added initializations and reordered
...
void sample_app::QnnSampleApp::displayKPI() {
  if (m_needKpi) {
    m_model->displayKPI();
  }
}
...
sample_app::StatusCode sample_app::QnnSampleApp::initialize() {
  ...
  if (m_dumpOutputs && !::pal::FileOp::checkFileExists(m_outputPath) &&
    ...
    exitWithMessage("Could not create output directory: " + m_outputPath,
                    EXIT_FAILURE);
  }
#endif
  // Read Input File List
  bool readSuccess;
  std::tie(m_inputimgLists, m_inputNameToIndex, readSuccess) =
      readInputArgsForSingleGraph(m_model, m_inputArgs);
  if (!readSuccess) {
  ...
  if (log::isLogInitialized()) {
    ...
    if (QNN_SUCCESS != m_qnnFunctionPointers.qnnInterface.logCreate(
                           logCallback, logLevel, &m_logHandle)) {
...
sample_app::StatusCode sample_app::QnnSampleApp::setPowerConfig() {
  ...
  powerConfig.dcvsV3Config.dcvsEnable =
      0; // True to enable Dcvs, False to disbale
  powerConfig.dcvsV3Config.setDcvsEnable = 1;
  powerConfig.dcvsV3Config.contextId =
      powerConfigId; // use the power config id created
  ...
  powerConfig.dcvsV3Config.sleepDisable =
      1; // True to disable sleep, False to re-enable sleep
  powerConfig.dcvsV3Config.setSleepDisable =
      1; // True to consider sleep disable/enable parameter otherwise False

  // Set Sleep latency parameter
  powerConfig.dcvsV3Config.sleepLatency =
      40; // set dsp sleep latency ranges 10-65535 micro sec, refer hexagon sdk
...
sample_app::StatusCode sample_app::QnnSampleApp::destroyPowerConfigId() {
  ...
  Qnn_ErrorHandle_t perfInfraErr =
      perfInfra.destroyPowerConfigId(powerConfigId);
  if (perfInfraErr != QNN_SUCCESS) {
...
sample_app::StatusCode sample_app::QnnSampleApp::createDevice() {
  if (nullptr != m_qnnFunctionPointers.qnnInterface.deviceCreate) {
    auto qnnStatus = m_qnnFunctionPointers.qnnInterface.deviceCreate(
        m_logHandle, nullptr, &m_deviceHandle);
...
sample_app::StatusCode sample_app::QnnSampleApp::executeGraphs() {
  ...
  for (size_t graphIdx = 0; graphIdx < m_graphsCount; graphIdx++) {
    ...
    if (graphIdx >= m_inputimgLists.size()) {
    ...
    auto inputimgList = m_inputimgLists[graphIdx];
    auto graphInfo = (*m_graphsInfo)[graphIdx];
    if (!inputimgList.empty()) {
      size_t totalCount = inputimgList[0].size();
      size_t inputImgIndexOffset = 0;
      while (inputImgIndexOffset < totalCount) {
        m_model->execution_start_times.push_back(
            std::chrono::high_resolution_clock::now());
        iotensor::StatusCode iotReturnStatus;
        size_t numInputImgsPopulated;
        size_t batchSize;
        std::tie(iotReturnStatus, numInputImgsPopulated, batchSize) =
            m_ioTensor.populateInputTensors(m_model, graphIdx, inputimgList,
                                            inputImgIndexOffset, false,
                                            m_inputNameToIndex[graphIdx],
                                            inputs, graphInfo, m_inputDataType);
        if (iotensor::StatusCode::SUCCESS != iotReturnStatus) {
          returnStatus = StatusCode::FAILURE;
        }
        if (StatusCode::SUCCESS == returnStatus) {
          QNN_DEBUG("Successfully populated input tensors for graphIdx: %d",
                    graphIdx);
...
#ifndef __hexagon__
            if (iotensor::StatusCode::SUCCESS !=
                m_ioTensor.writeOutputTensors(
                    m_model, graphIdx, inputImgIndexOffset, graphInfo.graphName,
                    outputs, graphInfo.numOutputTensors, m_outputDataType,
                    m_graphsCount, m_outputPath, numInputImgsPopulated,
                    batchSize)) {
              returnStatus = StatusCode::FAILURE;
            }
#endif
          }
          inputImgIndexOffset += numInputImgsPopulated;
        }
        if (StatusCode::SUCCESS != returnStatus) {
          QNN_ERROR("Execution of Graph: %d failed!", graphIdx);
          break;
        }
        m_model->execution_stop_times.push_back(
            std::chrono::high_resolution_clock::now());
      }
    }
