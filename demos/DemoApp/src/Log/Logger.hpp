...
#define QNN_ERROR(fmt, ...)                                                    \
  QNN_LOG_LEVEL(QNN_LOG_LEVEL_ERROR, fmt, ##__VA_ARGS__)
...
#define QNN_DEBUG(fmt, ...)                                                    \
  QNN_LOG_LEVEL(QNN_LOG_LEVEL_DEBUG, fmt, ##__VA_ARGS__)

#define QNN_VERBOSE(fmt, ...)                                                  \
  QNN_LOG_LEVEL(QNN_LOG_LEVEL_VERBOSE, fmt, ##__VA_ARGS__)

#define QNN_FUNCTION_ENTRY_LOG                                                 \
  QNN_LOG_LEVEL(QNN_LOG_LEVEL_VERBOSE, "Entering %s", __func__)

#define QNN_FUNCTION_EXIT_LOG                                                  \
  QNN_LOG_LEVEL(QNN_LOG_LEVEL_VERBOSE, "Returning from %s", __func__)
...
public:
  ...
  QnnLog_Level_t getMaxLevel() {
    return m_maxLevel.load(std::memory_order_seq_cst);
  }

  QnnLog_Callback_t getLogCallback() const { return m_callback; }
...
private:
  Logger(QnnLog_Callback_t callback, QnnLog_Level_t maxLevel,
         const QnnLog_Error_t *status);
