...
std::shared_ptr<Logger> Logger::createLogger(QnnLog_Callback_t callback,
  ...
  if (!s_logger) {
    s_logger = std::shared_ptr<Logger>(new (std::nothrow)
                                           Logger(callback, maxLevel, status));
  }
  *status = QNN_LOG_NO_ERROR;
  return s_logger;
}

Logger::Logger(QnnLog_Callback_t callback, QnnLog_Level_t maxLevel,
...
void Logger::log(QnnLog_Level_t level, const char *file, long line,
                 const char *fmt, ...) {
  if (m_callback) {
...
QnnLog_Callback_t qnn::log::getLogCallback() {
  return g_logger->getLogCallback();
}
