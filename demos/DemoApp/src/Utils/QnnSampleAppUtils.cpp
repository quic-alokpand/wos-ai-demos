...
void sample_app::parseInputFilePaths(const std::vector<std::string> &inputFilePaths,
                                     std::vector<std::string> &paths,
                                     std::string separator) {
  for (const auto& inputInfo : inputFilePaths) {
...
sample_app::readInputArgsForSingleGraph(Model *model,
                                        const std::vector<std::string>& inputArgs) {

  std::vector<std::vector<std::vector<std::string>>> filePathsForGraphs;
  ...
  std::tie(filePathList, inputNameToIndex, readSuccess) =
      readInputArgs(model, inputArgs);
  if (!readSuccess) {
    filePathsForGraphs.clear();
    return std::make_tuple(filePathsForGraphs, inputNameToIndexMaps, false);
  }
  filePathsForGraphs.push_back(filePathList);
  inputNameToIndexMaps.push_back(inputNameToIndex);
  return std::make_tuple(filePathsForGraphs, inputNameToIndexMaps, true);
}

sample_app::ReadInputListRetType_t
sample_app::readInputArgs(Model *model,
                          const std::vector<std::string>& inputArgs) {
  std::queue<std::string> args;
  for (auto const &arg : inputArgs) {
    if (arg.empty())
      continue;
    args.push(arg);
  }

  std::vector<std::vector<std::string>> imgFilesList;
  std::unordered_map<std::string, uint32_t> inputNameToIndex;
  if (!args.empty()) {
    inputNameToIndex = model->getInputNameToIndexMap();
  }
  while (!args.empty()) {
    std::vector<std::string> inputFiles;
    split(inputFiles, args.front(), ',');
    imgFilesList.reserve(imgFilesList.size());
    for (size_t idx = 0; idx < inputFiles.size(); idx++) {
      if (idx >= imgFilesList.size()) {
        imgFilesList.push_back(std::vector<std::string>());
      }
      imgFilesList[idx].push_back(inputFiles[idx]);
    }
    args.pop();
  }
  return std::make_tuple(imgFilesList, inputNameToIndex, true);
}

static std::string sample_app::sanitizeTensorName(std::string name) {
