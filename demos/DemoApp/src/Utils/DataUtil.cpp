...
std::tuple<datautil::StatusCode, size_t>
datautil::calculateLength(const std::vector<size_t>& dims, Qnn_DataType_t dataType) {
  if (dims.size() == 0) {
...
datautil::ReadBatchDataRetType_t
datautil::readBatchData(Model *model, const std::vector<std::string> &filePaths,
                        const size_t fileIndex, const bool loopBackToStart,
                        const std::vector<size_t> &dims,
                        const Qnn_DataType_t dataType, uint8_t *buffer,
                        size_t inputIndex, std::string inputNodeName) {
  if (nullptr == buffer) {
    QNN_ERROR("buffer is nullptr");
    return std::make_tuple(StatusCode::INVALID_BUFFER, 0, 0);
  }
  PreProcessContext context(buffer, filePaths, fileIndex, inputIndex,
                            inputNodeName, dims);
  model->preProcess(context);
  return std::make_tuple(StatusCode::SUCCESS, 1, 1);
}
...
datautil::StatusCode
datautil::writeBatchDataToFile(const std::vector<std::string>& fileDirs,
  ...
  for (size_t batchIndex = 0; batchIndex < fileDirs.size(); batchIndex++) {
    ...
    std::ofstream os(outputPath, std::ofstream::binary);
    if (!os) {
      QNN_ERROR("Failed to open output file for writing: %s",
                outputPath.c_str());
      return StatusCode::FILE_OPEN_FAIL;
    }
    for (size_t l = 0; l < outputSize; l++) {
      //size_t bufferIndex = l + (batchIndex * outputSize);
      os.write(reinterpret_cast<const char *>(buffer + l + (batchIndex * outputSize)), 1);
    }
  }
  return StatusCode::SUCCESS;
}
...
datautil::StatusCode datautil::writeBinaryToFile(std::string fileDir,
  ...
  os.write(reinterpret_cast<const char *>(buffer), bufferSize);
  return StatusCode::SUCCESS;
}
#endif
