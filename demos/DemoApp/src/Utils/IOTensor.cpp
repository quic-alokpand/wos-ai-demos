...
iotensor::IOTensor::readDataAndAllocateBuffer(
  ...
  *bufferToCopy = nullptr;
  ...
  std::tie(status, numFilesPopulated, batchSize) = datautil::readBatchData(
      model, filePaths, filePathsIndexOffset, loopBackToStart, dims, dataType,
      reinterpret_cast<uint8_t *>(*bufferToCopy), inputIndex, inputNodeName);
...
iotensor::PopulateInputTensorsRetType_t iotensor::IOTensor::populateInputTensor(
    Model *model, const std::vector<std::string> &filePaths,
    const size_t filePathsIndexOffset, const bool loopBackToStart,
    Qnn_Tensor_t *input, iotensor::InputDataType inputDataType,
    size_t inputIndex, const std::string &inputNodeName) {
  if (nullptr == input) {
  ...
  if (inputDataType == InputDataType::FLOAT &&
    ...
    std::tie(returnStatus, numFilesPopulated, batchSize) =
        readDataAndAllocateBuffer(model, filePaths, filePathsIndexOffset,
                                  loopBackToStart, dims, QNN_DATATYPE_FLOAT_32,
                                  &fileToBuffer, inputIndex, inputNodeName);
    if (StatusCode::SUCCESS == returnStatus) {
...
iotensor::PopulateInputTensorsRetType_t
iotensor::IOTensor::populateInputTensors(
  ...
  if (filePathsVector.size() != inputCount) {
    QNN_ERROR("Incorrect amount of Input files for graphIdx: %d. Expected: %d, "
              "received: %d",
  ...
  model->preprocess_start_times.push_back(
  ...
  for (size_t inputIdx = 0; inputIdx < inputCount; inputIdx++) {
    ...
    std::tie(returnStatus, currentInputNumFilesPopulated,
             currentInputNumBatchSize) =
        populateInputTensor(model, filePathsVector[inputNameIdx],
                            filePathsIndexOffset, loopBackToStart,
                            &(inputs[inputIdx]), inputDataType, inputIdx,
                            inputNodeName);
    if (StatusCode::SUCCESS != returnStatus) {
      QNN_ERROR(
          "populateInputTensorFromFiles failed for input %s with index %d",
          inputNodeName.c_str(), inputIdx);
      return {StatusCode::FAILURE, currentInputNumFilesPopulated,
              currentInputNumBatchSize};
    }
    if (inputIdx == 0) {
      ...
      if (numFilesPopulated != currentInputNumFilesPopulated ||
          numBatchSize != currentInputNumBatchSize) {
        QNN_ERROR("Current input tensor with name: %s with index %d files "
                  "populated = %d, batch size = %d"
                  " does not match with expected files populated = %d, batch "
                  "size = %d",
                  inputNodeName.c_str(), inputIdx,
                  currentInputNumFilesPopulated, currentInputNumBatchSize,
                  numFilesPopulated, numBatchSize);
        return {StatusCode::FAILURE, numFilesPopulated, numBatchSize};
      }
    }
  }
  model->preprocess_stop_times.push_back(
      std::chrono::high_resolution_clock::now());
...
iotensor::StatusCode
iotensor::IOTensor::allocateBuffer(uint8_t **buffer, const std::vector<size_t> &dims,
  ...
  case QNN_DATATYPE_INT_8:
    ...
    returnStatus = allocateBuffer<int8_t>(reinterpret_cast<int8_t **>(buffer),
                                          elementCount);
    break;
...
iotensor::StatusCode iotensor::IOTensor::convertAndWriteOutputTensorInFloat(
  ...
  uint8_t const *bufferToWrite = reinterpret_cast<uint8_t *>(floatBuffer);
  if (datautil::StatusCode::SUCCESS !=
...
iotensor::StatusCode iotensor::IOTensor::writeOutputTensor(
    Qnn_Tensor_t *output, const std::vector<std::string> &outputPaths,
    const std::string &fileName, size_t outputBatchSize) {
  if (nullptr == output) {
    QNN_ERROR("output is nullptr");
...
iotensor::StatusCode iotensor::IOTensor::writeOutputTensors(
    Model *model, uint32_t graphIdx, size_t startIdx, char *graphName,
  ...
  model->postprocess_start_times.push_back(
      std::chrono::high_resolution_clock::now());
  PostProcessContext context(this, graphIdx, startIdx, graphName, outputs,
                             numOutputs, graphsCount, outputPath,
                             numInputFilesPopulated, outputBatchSize);
  bool isSuccess = model->postProcess(context);
  model->postprocess_stop_times.push_back(
      std::chrono::high_resolution_clock::now());
  if (isSuccess) {
    return StatusCode::SUCCESS;
  }
