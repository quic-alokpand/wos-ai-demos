...
static inline T resolveSymbol(void *libHandle, const char *sym) {
  ...
  if (ptr == nullptr) {
    QNN_ERROR(
        "Unable to access symbol [%s]. pal::dynamicloading::dlError(): %s", sym,
...
dynamicloadutil::StatusCode dynamicloadutil::getQnnFunctionPointers(
    const std::string& backendPath, const std::string& modelPath,
    sample_app::QnnFunctionPointers *qnnFunctionPointers,
    void **backendHandleRtn, bool loadModelLib, void **modelHandleRtn) {
  ...
  if (nullptr == interfaceProviders) {
    QNN_ERROR("Failed to get interface providers: null interface providers "
              "received.");
    return StatusCode::FAIL_GET_INTERFACE_PROVIDERS;
  }
  if (0 == numProviders) {
...
dynamicloadutil::StatusCode dynamicloadutil::getQnnSystemFunctionPointers(
  ...
  if (nullptr == systemLibraryHandle) {
    QNN_ERROR(
        "Unable to load system library. pal::dynamicloading::dlError(): %s",
        pal::dynamicloading::dlError());
  ...
  if (nullptr == systemInterfaceProviders) {
    QNN_ERROR("Failed to get system interface providers: null interface "
              "providers received.");
    return StatusCode::FAIL_GET_INTERFACE_PROVIDERS;
  }
  if (0 == numProviders) {
